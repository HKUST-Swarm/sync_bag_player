...
class SyncBagPlayer:
    def __init__(self, bag_path, play_t0_sys=None):
        print("Starting sync player.. prepare for publishers...")
        ...
        self.lc = lcm.LCM()
        self.ctrl_sub = self.lc.subscribe("sync_bag_ctrl", self.sync_bag_ctrl_handle)
        self.t = threading.Thread(target = self.lcm_thread)
        self.t.start()
        print("Lcm Ready")
    
    def lcm_thread(self):
        print("Waiting for lcm commands")
        while True:
            self.lc.handle()
    ...
    def sync_bag_ctrl_handle(self, channel, msg):
        print(msg)
        if msg.cmd < 0:
            print("Program is cancelled by lcm, exiting")
            exit()
